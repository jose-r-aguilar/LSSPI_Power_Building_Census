...
# Define tables for Black, White, Asian, and Latino populations
racial_tables <- list(
  "Black" = "B01001B",
  "White" = "B01001A",
  "Asian" = "B01001D",
...
# Function to load and clean data for a given race and county
load_clean_data <- function(race, table, county) {
  get_acs(
    geography = "county",
    state = state,
    county = county,
    ...
    mutate(
      ...
      county = county  # Add county as a column
    ) %>%
    group_by(race, gender, age_group, county) %>%  # Group by race, gender, age group, and county
    summarize(total = sum(estimate, na.rm = TRUE), .groups = "drop")  # Summarize the total estimates
}

# Create separate tables for each county
for (county in counties) {
  county_data <- bind_rows(
    lapply(names(racial_tables), function(race) {
      load_clean_data(race, racial_tables[[race]], county)
    })
  )
  
  # Save the county-specific data to a CSV file
  file_name <- paste0("B01001_", county, "_race_age_gender.csv")
  write_csv(county_data, file_name)
  
  # Print a message for each county file created
  print(paste("Saved data for", county, "to", file_name))
}
